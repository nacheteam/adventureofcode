INSTRUCTIONS = """uz inc 134 if hx > -10
qin dec -300 if h <= 1
...
for instruction in instruction_list:
    ...
    if "dec" in instruction:
        register_key = instruction.split('dec')[0].strip()
    ...
    for i in range(len(conditionals)):
        if conditionals[i] in instruction:
            conditional_index = i
    conditional_register_key = condition.split(conditionals[conditional_index])[0].strip()
    conditional_register_value = condition.split(conditionals[conditional_index])[1].strip()
    value_to_inc_dec = 0
    inc = True
    if "inc" in instruction:
        inc = True
        value_to_inc_dec = instruction.split('inc')[1].strip().split('if')[0].strip()
    if "dec" in instruction:
        inc = False
        value_to_inc_dec = instruction.split('dec')[1].strip().split('if')[0].strip()
    try:
        index_register = register_keys.index(conditional_register_key)
    except ValueError:
        register_keys.append(conditional_register_key)
        register_values.append(0)
        index_register=-1
    if conditional_index==0:
        if int(register_values[index_register])==int(conditional_register_value):
            if inc:
                register_values[-1]+=int(value_to_inc_dec)
            else:
                register_values[-1]-=int(value_to_inc_dec)
    elif conditional_index==1:
        if int(register_values[index_register])!=int(conditional_register_value):
            if inc:
                register_values[-1]+=int(value_to_inc_dec)
            else:
                register_values[-1]-=int(value_to_inc_dec)
    elif conditional_index==2:
        if int(register_values[index_register])<int(conditional_register_value):
            if inc:
                register_values[-1]+=int(value_to_inc_dec)
            else:
                register_values[-1]-=int(value_to_inc_dec)
    elif conditional_index==3:
        if int(register_values[index_register])>int(conditional_register_value):
            if inc:
                register_values[-1]+=int(value_to_inc_dec)
            else:
                register_values[-1]-=int(value_to_inc_dec)
    elif conditional_index==4:
        if int(register_values[index_register])<=int(conditional_register_value):
            if inc:
                register_values[-1]+=int(value_to_inc_dec)
            else:
