PROGRAMS = """wdysq (135) -> sxldvex, wiasj
...
for line in lines:
    all_programs.append(line.split(' ')[0])
    aux_holded = []
    if "->" in line:
        holded_string = line.split('-> ')[1]
        if holded_string!="":
            aux_holded = holded_string.split(',')
            for holded in aux_holded:
                if " " in holded:
                    holded_programs.append(holded.split(' ')[1])
                else:
